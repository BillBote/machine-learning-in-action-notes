...
## 8.4 缩减系数来“理解”数据
在面对数据特征多于数据，即高维数据时，我们将无法计算$(X'X)^{-1}$，因此需要一些特殊方法来解决这个问题。

### 8.4.1 岭回归
首先引入岭回归的概念，我们已经知道标准线性回归的系数为$(X’X)^{-1}X’y$，那么岭回归的系数为

$
\hat{w}=(X^TX+\lambda{I})^{-1}X^Ty
$

我们可以理解为岭回归在误差函数中引入了惩罚项来减少不必要的参数，可以证明岭回归和$min\{||y-X\beta||^2+\lambda||\beta||^2\}$等价。这种方法称为缩减（shrinkage）。我们取不同的$\lambda$对训练集进行训练，对不同的$\lambda$找出使预测误差最小的作为最终的参数。

在regression.py文件中有岭回归函数(ridgeRegress和ridgeTest),同时利用matplotlib我们可以绘制出岭迹图，通过岭迹图直观找到适合的$\lambda$。

### 8.4.2 lasso
可以证明，当引入以下约束时可以得到和岭回归相同的公式（我们在上一节已经提到过岭回归的这种理解方法）：

$
\sum_{k=1}^nw_k^2\geq\lambda
$

即规定了所有回归系数的平方和不能大于$\lambda$，岭回归用这种限制避免了普通最小二乘回归时得到一个很大的正系数和一个很大的负系数的情况。

而lasso采用了很类似的约束来对回归系数做限定：

$
\sum_{k=1}^n|w_k|\geq\lambda
$

唯一的不同就是lasso对绝对值做了约束，但取得了和岭回归不同的结果：当$\lambda$足够小时，一些系数会被迫缩减到0。lasso就是取L1范数作为惩罚项，而我们知道L1范数可以实现稀疏化，这也就是我们避免过拟合需要实现的目标。而岭回归则是取L2范数作为惩罚项，两者细微的差别，却造成了计算复杂度的巨大变化，lasso解出回归系数需要使用二次规划算法，下一节将介绍一个更简单的方法——前向逐步回归。

### 8.4.3 前向逐步回归
前行逐步回归（不同于逐步回归，后者是用于选择变量）是一种贪心算法，开始设定变量权重相同，不妨为1，每一步对某个权重做一点小变化，增加或减少一个小值，选择使误差最小的权重。这种想法很自然，目的是为了找出使得误差最小的权重，但遍历所有权重是很困难的，但是如果一次只改变一个参数的权重的话是很轻松的，那么我们采用贪心策略，不再只考虑所有参数（那想的人头大），而是一次修改一个权重，找出其最小误差对应的权重，然后不断迭代，只要使得误差不断下降，就能得到非常好的结果。

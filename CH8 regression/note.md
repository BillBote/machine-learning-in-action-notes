...
唯一的不同就是lasso对绝对值做了约束，但取得了和岭回归不同的结果：当$\lambda$足够小时，一些系数会被迫缩减到0。lasso就是取L1范数作为惩罚项，而我们知道L1范数可以实现稀疏化，这也就是我们避免过拟合需要实现的目标。而岭回归则是取L2范数作为惩罚项，两者细微的差别，却造成了计算复杂度的巨大变化，lasso解出回归系数需要使用二次规划算法，下一节将介绍一个更简单的方法——向前回归。
